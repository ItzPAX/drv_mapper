...
public:
	...
	wnbios_lib()
	{
		...
		hHandle = CreateFileA("\\\\.\\WNBIOS", GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	...
	uintptr_t get_process_id(const wchar_t* image_name);
	uintptr_t get_process_base(const wchar_t* image_name);

	uintptr_t get_system_dirbase();
	uintptr_t get_ntos_vba();
	...
	uintptr_t get_pt(uint64_t v_address);
